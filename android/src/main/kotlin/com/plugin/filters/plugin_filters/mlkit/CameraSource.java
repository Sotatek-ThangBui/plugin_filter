...
import com.plugin.filters.plugin_filters.mlkit.processor.VisionImageProcessor;
...
public class CameraSource {
    ...
    public CameraSource(Context activity, GraphicOverlay overlay) {
    ...
    public synchronized CameraSource start() throws IOException {
        ...
        Log.d(TAG, "start:  run to start camera ");
        camera.startPreview();
        Log.d(TAG, "start:  run to start camera 111");

        processingThread = new Thread(processingRunnable);
        processingRunnable.setActive(true);
        processingThread.start();
        Log.d(TAG, "createCamera:  size pair is null  ");
    ...
    public List<SizePair> getResolutions() {
        return commonResolution();
//        Map map = new HashMap<String, SizePair>();
//        List<SizePair> resolutions = commonResolution();
//        for (int i = 0; i < resolutions.size(); i++) {
//            long pixelCountTemp = resolutions.get(i).preview.getHeight() * resolutions.get(i).preview.getWidth();
//            float resolution = ((float) pixelCountTemp) / (1024000.0f);
//            map.put(resolution, resolutions.get(i));
//        }
//        return map;
    }


    public List<SizePair> commonResolution() {
        if (camera == null) {
            return new ArrayList<>();
        }
        SizePair high = selectSizePair(camera, 1920, 1080);
        SizePair medium = selectSizePair(camera, 1280, 720);
//        SizePair sizePair4 = selectSizePair(camera, 1420, 1080);
        SizePair low = selectSizePair(camera, DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH,
                DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT);
//        SizePair sizePair3 = selectSizePair(camera, 800, 500);
        return Arrays.asList(low, medium, high);
    }

    public void setResolution(SizePair sizePair) {

        PreferenceUtils.setCameraPreviewSizePair(activity, getIdForRequestedCamera(facing), sizePair);
    }
...
//    }
        ...
        if (sizePair == null) {
            sizePair =
                    selectSizePair(
                            ...
                            DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT);
            Log.d(TAG, "createCamera:  size pair is null " + new Gson().toJson(sizePair));
        }
    ...
    public static List<SizePair> generateValidPreviewSizeList(Camera camera) {
        ...
        for (Camera.Size previewSize : supportedPreviewSizes) {
            ...
            Log.d(TAG, "generateValidPreviewSizeList: preview  " + previewAspectRatio);
            // By looping through the picture sizes in order, we favor the higher resolutions.
            // We choose the highest resolution in order to support taking the full resolution
            // picture later.
            for (Camera.Size pictureSize : supportedPictureSizes) {
                float pictureAspectRatio = (float) pictureSize.width / (float) pictureSize.height;
//                Log.d(TAG, "generateValidPreviewSizeList:  " + pictureAspectRatio);
